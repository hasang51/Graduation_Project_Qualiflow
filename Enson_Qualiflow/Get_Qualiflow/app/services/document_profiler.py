...
def _extract_text_stats(pdf_path: Path) -> tuple[int, float, bool]:
    """Return ``(page_count, text_density, has_text_layer)``.
    ...
    for page in reader.pages:
        ...
        chars = len(stripped)
        total_chars += chars
        # A scan might still return a handful of OCR-free characters from a
        # font-embedded header. Require a reasonable floor.
        if chars >= 40:
            non_trivial_pages += 1
...
def _classify(
    *,
    ...
    if has_text_layer and text_density >= DIGITAL_TEXT_DENSITY_MIN:
        # Text layer present but image quality is borderline; treat as scan_clean
...
def profile_document(pdf_path: str | Path, *, document_id: str | None = None) -> DocumentProfile:
    ...
    page_count, text_density, has_text_layer = _extract_text_stats(path)
