...
def _match_single(normalized_token: str) -> _CanonicalGrade | None:
    ...
    if not normalized_token:
        return None
    key = _alias_key(normalized_token)
    grade = _alias_index().get(key)
    if grade is not None:
        return grade
    # Try without delivery condition suffix (``S355J2+N`` -> ``S355J2``).
    stripped = _strip_delivery_condition(normalized_token)
    if stripped and stripped != normalized_token:
        return _alias_index().get(_alias_key(stripped))
    return None
